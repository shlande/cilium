...
func Test_httpCluster_withUpstreamProxyProtocol(t *testing.T) {
	c := &cecTranslator{
		Config: Config{
			ClusterConfig: ClusterConfig{
				UseUpstreamProxyProtocol: true,
			},
		},
	}
	res, err := c.httpCluster("test-cluster", "test-cluster", false, "")
	require.NoError(t, err)
	...
	// Verify the nested Any can be round-tripped
	ppTransport := &envoy_transport_sockets_proxy_protocol_v3.ProxyProtocolUpstreamTransport{}
	err = cluster.TransportSocket.GetTypedConfig().UnmarshalTo(ppTransport)
	require.NoError(t, err)
	require.Equal(t, envoy_config_core_v3.ProxyProtocolConfig_V2, ppTransport.Config.Version)
...
func Test_httpCluster_withUpstreamProxyProtocol_disabled(t *testing.T) {
	c := &cecTranslator{
		Config: Config{
			ClusterConfig: ClusterConfig{
				UseUpstreamProxyProtocol: false,
			},
		},
	}
	res, err := c.httpCluster("test-cluster", "test-cluster", false, "")
	...
	require.Nil(t, cluster.TransportSocket, "TransportSocket must be nil when UseUpstreamProxyProtocol=false")
}
